...
{  
    ...
    SEXP eobmax(SEXP num_verts_in, SEXP num_edges_in, SEXP R_edges_in, SEXP R_weights_in )
    {
        using namespace boost;

        Graph_dd g(num_verts_in, num_edges_in, R_edges_in, R_weights_in);
        property_map<Graph_dd, boost::edge_weight_t>::type weights = get(edge_weight, g);
	property_map<Graph_dd, boost::vertex_index_t>::type vertex_indices = get(vertex_index, g);
